...
| BL-45 | App crashes on window restore when WebView2 has focus | Click preview → minimize → restore crashes with `System.ArgumentException` in `ICoreWebView2Controller.MoveFocus()`. `IsTerminating=true` — unrecoverable. Round 1 (`Deactivated` focus transfer) failed — fires too late. Round 2 fix: (1) `Focusable="False"` on `NativeWebView` in `MainWindow.axaml` — removes WebView from Avalonia's focus chain entirely so `SetFocusScope` never routes `MoveFocus` through it. Primary fix. (2) `Activated` handler posts `_editor.Focus()` at `DispatcherPriority.Input` as belt-and-suspenders. (3) `Dispatcher.UIThread.UnhandledException` narrow backstop retained (match on type + message + MoveFocus in stack). Click-to-sync unaffected — `Focusable` controls logical focus only, not hit-testing or JS `WebMessageReceived`. | ✅ Complete |
...
*Last updated: BL-45 complete — WebView2 focus crash on window restore fixed via Focusable=False.*